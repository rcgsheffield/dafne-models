...
def liver_surface_apply(modelObj: DynamicDLModel, data: dict):
    try:
...
def liver_surface_incremental_mem(modelObj: DynamicDLModel, trainingData: dict, trainingOutputs,
    ...
    try:
        ...
        from dafne_dl.labels.liver_surface import inverse_labels
    except ModuleNotFoundError:
        import dl.common.preprocess_train as pretrain
        from dl.common.DataGenerators import DataGeneratorMem
        from dl.labels.liver_surface import inverse_labels
    ...
    BAND = 64
    BATCH_SIZE = bs
    # CHECKPOINT_PATH = os.path.join(".", "Weights_incremental_split", "liver_surface")
    ...
    print(f'Incremental learning for liver surface with {nImages} images')
    t = time.time()
...
generate_convert(model_id='ba333b4d-90e7-4108-aca5-9216f408d91e',
                 default_weights_path=os.path.join('weights', 'weights_liver_surface_split.hdf5'),
                 model_name_prefix='liver_surface',
                 model_create_function=liver_surface_unet,
                 model_apply_function=liver_surface_apply,
                 model_learn_function=liver_surface_incremental_mem
                 )
